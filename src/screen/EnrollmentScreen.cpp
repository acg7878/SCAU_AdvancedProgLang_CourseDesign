...
void ModifyEnrollmentScreen(std::vector<Enrollment> &enrollments) {
  ...
  auto modify_button = Button("修改", [&] {
    try {
      ...
      bool success = update_enrollment_grade(enrollments, studentId, courseId,
                                             examGrade, usualGrade);
  ...
  auto renderer = Renderer(container, [&] {
    return vbox({
               ...
               separator(),
               hbox(modify_button->Render() | center,
                    back_button->Render() | center) |
                   center,
               separator(),
               text(message) | color(Color::Red) | center,
...
void DeleteEnrollmentScreen(std::vector<Enrollment> &enrollments) {
  ...
  auto renderer = Renderer(container, [&] {
    return vbox({
               ...
               student_input->Render() | center,
               course_input->Render() | center,
               hbox(delete_button->Render() | center,
                    back_button->Render() | center) |
                   center,
               separator(),
               text(message) | color(Color::Red) | center,
...
void QueryFailingListScreen(const std::vector<Enrollment> &enrollments,
      ...
      Button("查询",
             [&] {
               try {
                 ...
                 for (const auto &[studentName, courseName, grade] :
                      failing_courses) {
  ...
  auto renderer = Renderer(comp, [&] {
    ...
    if (selected == 1) {
      int now = 0;
      for (const auto &[student_id, studentName, courseName, grade] :
           fail_all) {
        if (now / 8 == page) {
    ...
    try {
      ...
      for (const auto &enrollment : enrollments) {
        if (enrollment.getCourseId() == courseID) {
          ...
          auto course = std::find_if(
              courses.begin(), courses.end(), [&](const Course &c) {
          ...
          if (student != students.end() && course != courses.end()) {
            ...
            el_by_id[3].push_back(
                text(std::to_string(enrollment.getCompositeGrade())) | center);
          }
        }
      }
  ...
  auto renderer = Renderer(comp, [&] {
    ...
    if (selected == 1) {
      ...
      if (!enrollmentFound && selected == 2) {
        content.push_back(text("未查询到选课信息") | color(Color::Red) |
                          center);
      }
      return vbox(content) | border | size(WIDTH, EQUAL, 80) | center;
    }
