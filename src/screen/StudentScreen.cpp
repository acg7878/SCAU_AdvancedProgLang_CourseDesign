...
void PrintStudentInfo(std::vector<Student> &students) {
  ...
  // 输入框和查询按钮
  auto input_box =
      Input(&studentID, "请输入学生ID") | size(ftxui::HEIGHT, ftxui::EQUAL, 1);
  auto submit_button =
      Button("查询",
             [&] {
               try {
                 ...
                 el_by_id[0].push_back(vbox(text(student.getId())) | flex |
                                       center);
                 el_by_id[1].push_back(vbox(text(student.getName())) | flex |
                                       center);
                 el_by_id[2].push_back(
                     vbox(text(student.getSex() == 0 ? "男" : "女")) | flex |
                     center);
                 el_by_id[3].push_back(
                     vbox(text(std::to_string(student.getAge()))) | flex |
                     center);
                 el_by_id[4].push_back(
                     vbox(text(std::to_string(student.getDormNumber()))) |
                     flex | center);
                 el_by_id[5].push_back(vbox(text(student.getPhoneNumber())) |
                                       flex | center);
                 studentFound = true;
  ...
  auto renderer = Renderer(comp, [&] {
    ...
    if (selected == 1) {
      ...
      for (const auto &student : students) {
        if (now / 8 == page) {
          ...
          el_all[2].push_back(text(student.getSex() == 0 ? "男" : "女") |
                              center);
          el_all[3].push_back(text(std::to_string(student.getAge())) | center);
          el_all[4].push_back(text(std::to_string(student.getDormNumber())) |
                              center);
    ...
    if (selected == 1) {
      ...
      if (!studentFound && selected == 2) {
        content.push_back(text("未查询到学生信息") | color(Color::Red) |
                          center);
      }
...
void AddStudentScreen(std::vector<Student> &students) {
  auto screen = ScreenInteractive::TerminalOutput();
  std::string newStudentID, newStudentName, newStudentPhone, newStudentAge,
      newStudentDorm;
  int newStudentSex = 0;
  ...
  auto add_button = Button("录入", [&] {
    try {
      ...
      Student newStudent =
          createStudent(newStudentID, newStudentName, newStudentSex, age, dorm,
                        newStudentPhone);
...
void ShowStudentScreen(std::vector<Student> &students) {
  ...
  auto main_menu_renderer = Renderer(main_menu_container, [&] {
    return vbox({text("学生信息管理") | bold | center, separator(),
                 menu->Render() | center,
