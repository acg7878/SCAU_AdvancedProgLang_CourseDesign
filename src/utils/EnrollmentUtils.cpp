...
int count_failing_students_by_course_id(
  ...
  for (const auto &enrollment : enrollments) {
    if (enrollment.getCourseId() == courseId &&
        enrollment.getCompositeGrade() < 60) {
      count++;
    }
  }
...
get_failing_courses_by_student_id(const std::vector<Enrollment> &enrollments,
  ...
  for (const auto &enrollment : enrollments) {
    if (enrollment.getStudentId() == studentID &&
        enrollment.getCompositeGrade() < 60 && enrollment.getCompositeGrade() >= 0) {
      auto course =
          std::find_if(courses.begin(), courses.end(), [&](const Course &c) {
            return c.getId() == enrollment.getCourseId();
          });
      if (course != courses.end()) {
        result.emplace_back(studentName, course->getName(),
...
bool update_enrollment_grade(std::vector<Enrollment> &enrollments,
                             ...
                             int usualGrade) {
  auto enrollment =
      std::find_if(enrollments.begin(), enrollments.end(), [&](Enrollment &e) {
        return e.getStudentId() == studentId && e.getCourseId() == courseId;
      });
  ...
  if (enrollment != enrollments.end()) {
    ...
    enrollment->setCompositeGrade((examGrade + usualGrade) /
                                  2); // 假设综合成绩为两者平均
    DataLoader::saveEnrollments("../data/enrollment.json", enrollments);
    return true; // 成绩更新成功
  }
